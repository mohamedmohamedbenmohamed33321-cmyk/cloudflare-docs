...
:::note[HTTP interception only]
Containers only intercept HTTP traffic. HTTPS interception is not currently available.
...
:::caution
Hostnames that do not resolve via DNS do not work in local development. These hostnames do
work in production.
:::
