...
* `send(bodyunknown, options?{ contentType?: QueuesContentType })` <Type text="Promise<QueueSendResult>" />
...
* `sendBatch(messagesIterable<MessageSendRequest<unknown>>, options?QueueSendBatchOptions)` <Type text="Promise<QueueSendResult>" />
