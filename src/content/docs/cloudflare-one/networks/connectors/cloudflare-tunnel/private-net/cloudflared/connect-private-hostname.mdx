...
<Details header="Cloudflare Mesh">

1. In your [mesh node device profile](/cloudflare-one/networks/connectors/cloudflare-mesh/routes/), ensure that the <GlossaryTooltip term="initial resolved IP">initial resolved IP</GlossaryTooltip> listed above route through the tunnel.
2. Depending on where you installed the mesh node, you may also need to route those destination IPs through the node and point your DNS resolver to Cloudflare Gateway. Refer to [Routes](/cloudflare-one/networks/connectors/cloudflare-mesh/routes/).

</Details>

<Details header="Cloudflare WAN">

1. Ensure that the <GlossaryTooltip term="initial resolved IP">initial resolved IP</GlossaryTooltip> listed above [route through Cloudflare WAN](/cloudflare-wan/configuration/manually/how-to/configure-routes/) to Cloudflare.
2. [Point the DNS resolver](/cloudflare-wan/zero-trust/cloudflare-gateway/#dns-filtering) for your Cloudflare WAN network to Cloudflare Gateway.
