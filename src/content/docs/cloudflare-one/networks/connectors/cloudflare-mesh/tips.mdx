---
pcx_content_type: reference
title: Tips and best practices
tags:
  - Private networks
  - Mesh
sidebar:
  order: 6
---

import { Tabs, TabItem } from "~/components";

This page covers operational guidance for managing Cloudflare Mesh deployments, including how to update the agent, configure cloud provider settings, and troubleshoot common issues.

## Update a Mesh node

Updating a Mesh node requires updating the `cloudflare-warp` package on the Linux host. During the update, the node will briefly disconnect, which will interrupt traffic currently being routed through it. If you have [high availability](/cloudflare-one/networks/connectors/cloudflare-mesh/high-availability/) enabled, traffic will fail over to a standby replica.

<Tabs>
<TabItem label="Debian / Ubuntu">

1. Check the current version:

   ```sh
   warp-cli --version
   ```

2. Update the package:

   ```sh
   sudo apt-get update && sudo apt-get install --only-upgrade cloudflare-warp
   ```

</TabItem>
<TabItem label="RedHat / CentOS">

1. Check the current version:

   ```sh
   warp-cli --version
   ```

2. Update the package:

   ```sh
   sudo yum update cloudflare-warp
   ```

</TabItem>
</Tabs>

3. Verify the node has reconnected:

   ```sh
   warp-cli status
   ```

   You should see `Status update: Connected` in the output.

## Cloud VPC deployments

When deploying Mesh nodes in a cloud VPC, you may need to configure additional provider settings so the node can forward traffic for other devices on the subnet.

### Google Cloud Platform (GCP)

[Enable IP forwarding](https://cloud.google.com/vpc/docs/using-routes#canipforward) on the VM instance where you installed the Mesh node.

### Amazon Web Services (AWS)

- Disable [source/destination checking](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/using-eni.html) on the EC2 instance.
- In your [subnet route table](https://docs.aws.amazon.com/vpc/latest/userguide/subnet-route-tables.html), add a route for Mesh traffic (for example, `100.96.0.0/12`) pointing to the EC2 instance.

### Microsoft Azure

- [Enable IP forwarding](https://learn.microsoft.com/en-us/azure/virtual-network/virtual-network-network-interface?tabs=azure-portal#enable-or-disable-ip-forwarding) on the network interface of the VM.
- Add a [user-defined route](https://learn.microsoft.com/en-us/azure/virtual-network/manage-route-table) for Mesh traffic pointing to the VM's private IP.

## Running Mesh with cloudflared

A Mesh node and a [`cloudflared` tunnel](/cloudflare-one/networks/connectors/cloudflare-tunnel/private-net/cloudflared/) can run on the same Linux host. This is useful when you want to use the Mesh node as a gateway for your private network while also using `cloudflared` to publish specific applications.

The Mesh node captures all outbound traffic and routes it through Cloudflare by default. This can prevent `cloudflared` from making its [required outbound connections](/cloudflare-one/networks/connectors/cloudflare-tunnel/configure-tunnels/tunnel-with-firewall/#required-for-tunnel-operation). To resolve this, use [Split Tunnels](/cloudflare-one/team-and-resources/devices/cloudflare-one-client/configure/route-traffic/split-tunnels/) to exclude the Cloudflare Tunnel destinations from the WARP tunnel:

- `region1.v2.argotunnel.com`
- `region2.v2.argotunnel.com`
- `us-region1.v2.argotunnel.com` (US region only)
- `us-region2.v2.argotunnel.com` (US region only)

:::note
Split Tunnels is the only supported method of running both connectors on one machine. The Mesh node's kernel-level integration overrides routing configurations made by `ip route add` or `iptables`.
:::

## Source IPs for Cloudflare services

When Cloudflare services (such as [Load Balancing](/load-balancing/) health checks or [Workers](/workers/)) send traffic to your private network through a Mesh node, the traffic originates from the Cloudflare source IP range (default `100.64.0.0/12`). You may need to [configure Cloudflare source IPs](/cloudflare-one/networks/connectors/cloudflare-wan/configuration/manually/how-to/configure-cloudflare-source-ips/) to avoid IP conflicts.

## MTU and packet fragmentation

Mesh nodes use encapsulation to route traffic, which adds overhead to each packet. This is especially relevant for traffic between two Mesh participants, where the packet may be encapsulated twice (once by the sending node, and again by Cloudflare before delivery to the receiving side).

If source devices send packets near the maximum size (1,460 bytes or more), the double encapsulation can push packets over 1,500 bytes, causing them to be dropped.

### Recommendations

- Set the MTU on source devices (servers, cameras, IoT devices) to **1,280 bytes** to ensure packets fit after encapsulation.
- For TCP-only traffic, apply MSS clamping on your router with a value of **1,240 bytes** (1,280 MTU - 20 byte IP header - 20 byte TCP header).
- Modern applications using [Path MTU Discovery (PMTUD)](https://www.cloudflare.com/learning/network-layer/what-is-mtu/) typically handle this automatically.

## Gateway activity logs

Traffic from Mesh nodes appears in [Gateway activity logs](/cloudflare-one/insights/logs/gateway-logs/) with the identity `warp_connector@<your-team-name>.cloudflareaccess.com`. Client device traffic appears under the enrolled user's identity.
